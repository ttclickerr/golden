...
import { AnalyticsEvents, trackAllAnalytics } from "@/lib/analytics-universal";
...
import { getLanguageFlag, getLanguageName, useTranslation, useTranslationStore } from "@/lib/i18n";
...
import { Cloud, Globe, RotateCcw, Settings, Shield, Vibrate, Volume2 } from "lucide-react";
...
export default function TycoonClicker() {
    ...
    // Используем useRef для отслеживания предыдущего значения языка
    const prevLanguageRef = useRef(language);

    // Выводим лог только при изменении языка
    useEffect(() => {
    ...
    const prevPremiumStatusRef = useRef<boolean | null>(null);
    useEffect(() => {
        const handlePremiumChange = () => {
            ...
            const prevPremiumStatus = prevPremiumStatusRef.current;
            const newPremiumStatus = (savedPremiumType === 'lifetime' || savedPremiumType === 'weekly') && !testModeActive;

            // Логируем только при реальном изменении статуса
            if (prevPremiumStatus !== null && prevPremiumStatus !== newPremiumStatus) {
                console.log('🔍 Premium Status Change:', { savedPremiumType, testModeActive, prevPremiumStatus, newPremiumStatus });
            }
            prevPremiumStatusRef.current = newPremiumStatus;            // Показываем уведомление при изменении статуса премиума
            if (prevPremiumStatus !== null && !prevPremiumStatus && newPremiumStatus) {
                // Премиум только что активирован
                addNotification({
                    title: '🎉 ' + t('boosterActivatedNotif').replace('Бустер', 'Премиум').replace('Booster', 'Premium'),
                    message: t('premiumLifetimeDesc'),
                    type: 'success',
                    duration: 5000
                });
            } else if (prevPremiumStatus !== null && prevPremiumStatus && !newPremiumStatus) {
                // Премиум истек или отключен
                addNotification({
                    title: 'ℹ️ ' + t('boosterExpiredNotif').replace('Бустер', 'Премиум').replace('Booster', 'Premium'),
                    message: t('removeAds'),
                    type: 'info',
                    duration: 5000
                });
    ...
    useEffect(() => {
        const checkNewAchievements = () => {
            ...
            if (freshAchievements.length > 0) {
                ...
                // Получаем локализованное имя достижения для лога
                const category = mapAchievementCategory(latestAchievement.category);
                const n = latestAchievement.level;
                const value = latestAchievement.requirement?.value;
                const localized = getLocalizedAchievement(language, category, { n, value });
                console.log(`🏆 New Achievement: ${localized.name} - $${latestAchievement.reward} reward!`);
            }
        };
    ...
    const handleQuickBuyAssetLogged = (assetId: string) => {
        console.log(`💼 Quick buy request for: ${assetId}`);
        
        // Получаем актуальную цену актива
        let price = 0;
        
        // Для торговых активов используем динамические цены
        if (TRADING_ASSETS.some((a: any) => a.id === assetId)) {
            price = prices[assetId] || getTradingAssetPrice(assetId, undefined);
        } else {
            // Для остальных активов используем статические цены
        ...
        if (price > 0 && gameState.balance >= price) {
            console.log(`💼 Buying ${assetId} for $${price}`);
            const result = quickBuyAsset(assetId, price); // передаем правильную цену
            if (result !== undefined) {
                console.log(`✅ Successfully bought: ${assetId} for $${price}`);
                vibrationService.vibrateSuccess();
            }
            return result;
        } else {
            console.log(`❌ Cannot buy ${assetId}: insufficient funds or invalid price`);
            return undefined;
        }
    };
    ...
    // Вспомогательная функция для получения активов по категории
    const getAssetsByCategory = (category: string) => {
        return ASSETS.filter((asset: any) => asset.category === category);
    };    // Используем импортированную функцию mapAchievementCategory из achievement-localization.ts    // Используем функцию из общего модуля локализации для маппинга
    const getCompletedAchievements = () => {
        try {
            // Проверяем, существуют ли достижения в gameState
            if (!gameState.achievements || !Array.isArray(gameState.achievements)) {
                // console.warn('[i18n] gameState.achievements is undefined or not an array');
                return [];
            } // console.log('[i18n] gameState.achievements:', gameState.achievements);
            // console.log('[i18n] ACHIEVEMENTS array size:', ACHIEVEMENTS.length);

            // Подробное логирование для диагностики
            if (ACHIEVEMENTS.length > 0) {
                // console.log('[i18n] First achievement structure:', JSON.stringify(ACHIEVEMENTS[0]));
                // console.log('[i18n] Achievement categories:', ACHIEVEMENTS.map(a => a.category).slice(0, 5));
                // console.log('[i18n] Achievement levels:', ACHIEVEMENTS.map(a => a.level).slice(0, 5));
                // console.log('[i18n] Achievement requirements:', ACHIEVEMENTS.map(a => a.requirement?.type).slice(0, 5));
            }

            // Создаем тестовые достижения, если нет реальных
            if (gameState.achievements.length === 0) {
                // console.log('[i18n] No achievements in gameState, returning empty array');
                return [];
            }

            return ACHIEVEMENTS.filter((achievement: Achievement) => {                // Проверка на валидность достижения
                if (!achievement || !achievement.id || !achievement.category) {
                    // console.error('[i18n] Invalid achievement object:', achievement);
                    return false;
                }
                return gameState.achievements.includes(achievement.id);
            }).map((achievement: Achievement) => {                try {
                    // console.log('[i18n] Processing achievement:', achievement.id, achievement.category);

                    // Принудительно проверяем категорию
                    if (!achievement.category) {
                        // console.error('[i18n] Achievement has no category:', achievement);
                        achievement.category = 'wealth'; // Безопасное значение по умолчанию
                    }

                    const category = mapAchievementCategory(achievement.category);
                    const n = achievement.level || 1;

                    // Проверка на валидность требования
                    if (!achievement.requirement || achievement.requirement.value === undefined) {
                        // console.error('[i18n] Achievement has invalid requirement:', achievement);
                        achievement.requirement = { type: 'balance', value: 1000 }; // Безопасное значение по умолчанию
                    }

                    const value = achievement.requirement.value;

                    // console.log('[i18n] Achievement prepared for localization:', {
                    //     id: achievement.id,
                    //     category,
                    //     n,
                    //     value,
                    ...
                    // Проверки для защиты от undefined
                    const localized = getLocalizedAchievement(language, category, { n, value });
                    // console.log('[i18n] Localization result for', achievement.id, ':', localized);
                    ...
                    // console.error(`[i18n] Error localizing achievement ${achievement.id}:`, error);
                    return {
                        ...achievement,
                        name: `[Error: ${achievement.id}]`,
                        description: `Error during localization`
                    };
                }
            });
        } catch (error) {
            // console.error('[i18n] Critical error in getCompletedAchievements:', error);
            return [];
        }    }; const getPendingAchievements = () => {
        try {
            // console.log('[i18n] ACHIEVEMENTS in getPendingAchievements:', ACHIEVEMENTS.length);

            // Подробное логирование для диагностики
            if (ACHIEVEMENTS.length > 0) {
                // console.log('[i18n] First pending achievement structure:', JSON.stringify(ACHIEVEMENTS[0]));
                // console.log('[i18n] Pending achievement categories:', ACHIEVEMENTS.map(a => a.category).slice(0, 5));
                // console.log('[i18n] Pending achievement levels:', ACHIEVEMENTS.map(a => a.level).slice(0, 5));
                // console.log('[i18n] Pending achievement requirements:', ACHIEVEMENTS.map(a => a.requirement?.type).slice(0, 5));
            }            return ACHIEVEMENTS.filter((achievement: Achievement) => {
                if (!achievement || !achievement.id || !achievement.category) {
                    // console.error('[i18n] Invalid achievement in filter:', achievement);
                    return false;
                }
                return !gameState.achievements.includes(achievement.id);
            }).map((achievement: Achievement) => {
                try {
                    if (!achievement || !achievement.category || !achievement.level || !achievement.requirement) {
                        // console.error('[i18n] Invalid achievement structure in getPendingAchievements:', achievement);
                        return achievement;
                    }
                    ...
                    // console.log(`[i18n debug] Processing pending achievement ${achievement.id}:`, {
                    //     category,
                    ...
                    //     language
                    // });
                    if (category && n !== undefined && value !== undefined) {
                        const localized = getLocalizedAchievement(language, category, { n, value });
                        return {
                            ...achievement,
                            name: localized.name,
                    ...
                    }                    // Если не получилось локализовать, возвращаем оригинальные значения
                    return achievement;
                } catch (error) {
                    // console.error(`[i18n] Error localizing achievement ${achievement.id}:`, error);
                    return achievement;
            ...
            // console.error('[i18n] Critical error in getPendingAchievements:', error);
            return [];
        }
    };
        ...
        <div className="space-y-6">
            ...
            <Card className="glass-card border-white/10 p-4">
                ...
                <div className="space-y-3">                    {(() => {
                    // Создаём тестовые достижения для отладки
                    const testAchievements = [
                        ...
                        },
                        {
                            ...
                            level: 2,
                            requirement: { type: 'investmentValue', value: 5000 },
                            icon: 'fas fa-chart-line',
                            reward: 200
                        }
                    ];                    // Проверяем локализацию тестовых достижений
                    // console.log('[i18n] Testing wealth achievement localization:');
                    const testLocalizedWealth = getLocalizedAchievement(language, 'wealth', { n: 1, value: 1000 });
                    // console.log('[i18n] Wealth test result:', testLocalizedWealth);

                    // console.log('[i18n] Testing investment achievement localization:');
                    const testLocalizedInv = getLocalizedAchievement(language, 'investment', { n: 2, value: 5000 });
                    // console.log('[i18n] Investment test result:', testLocalizedInv);

                    // Проверка gameState.achievements
                    if (!gameState.achievements || gameState.achievements.length === 0) {
                        // console.log('[i18n] No achievements in gameState. Adding test achievements for display.');

                        // Возвращаем тестовые достижения для отображения
                        return testAchievements.map(achievement => {
                            const category = mapAchievementCategory(achievement.category);
                            const n = achievement.level;
                            const value = achievement.requirement?.value;
                            const localized = getLocalizedAchievement(language, category, { n, value });
                            ...
                            return (
                                <div key={achievement.id} className="flex items-center gap-3 p-3 glass-card rounded-lg border-2 border-[hsl(var(--tropico-gold))]">
                                    ...
                                    <div>
                                        <div className="font-medium text-[hsl(var(--tropico-gold))]">{localized.name}</div>
                                        <div className="text-sm text-gray-400">{localized.description}</div>
                                    </div>
                                    <Badge className="ml-auto gold-gradient text-black">{t('completed')}!</Badge>
                                </div>
                            );
                        });
                    }                    // Получаем реальные достижения
                    const realAchievements = getCompletedAchievements();
                    // console.log('[i18n] Real achievements:', realAchievements);
                    // console.log('[i18n] Achievement categories:', realAchievements.map(a => a.category));

                    return realAchievements.slice(-3).map((achievement: Achievement) => {
                        if (!achievement || !achievement.category) {
                            // console.error('[i18n] Invalid achievement:', achievement);
                            return null;
                        }

                        const category = mapAchievementCategory(achievement.category);
                        const n = achievement.level;
                        const value = achievement.requirement?.value;                        // console.log(`[i18n debug] Achievement ${achievement.id}:`, {
                        //     originalCategory: achievement.category,
                        //     mappedCategory: category,
                        //     level: n,
                        //     value: value,
                        //     language
                        ...
                        const localized = getLocalizedAchievement(language, category, { n, value });
                        // console.log(`[i18n debug] Localized result:`, localized);
    ...
    const renderPortfolioSection = () => {
        return (<PortfolioSection
            ...
            onQuickBuyAsset={handleQuickBuyAssetLogged}
            onQuickSellAsset={quickSellAsset}
        />
        );
    };
                    ...
                    <Card key={achievement.id} className="glass-card border-2 border-[hsl(var(--tropico-gold))] p-4">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex-1">
                                <h3 className="font-bold text-lg text-[hsl(var(--tropico-gold))]">{localized.name}</h3>
                                <p className="text-gray-400 text-sm">{localized.description}</p>
                ...
                <Card className="glass-card border-white/10 p-6">
                    ...
                    <div className="mb-6">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            {LANGUAGES.map(l => l.code).map((lang) => (
                                <button
                                    key={lang}
                                    onClick={() => handleLanguageChange(lang as Language)}
                                    className={`flex items-center gap-3 p-3 rounded-lg transition-all ${language === lang
                    ...
                    <div className="mb-6">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <button
                                ...
                                className={`flex items-center justify-center gap-3 p-4 rounded-lg transition-all ${soundEnabled
                            ...
                            <button
                                ...
                                className={`flex items-center justify-center gap-3 p-4 rounded-lg transition-all ${vibrateEnabled
                                    ? 'bg-gray-500/30 text-gray-300'
                                    : 'bg-gray-500/20 text-gray-400'
                                    }`}
                            >
