...
export function SettingsModal({ isOpen, onClose }: SettingsModalProps) {
  ...
  return (
    <div
      className={`fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 transition-opacity duration-200 ${isOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
      aria-hidden={!isOpen}
    >
            ...
            <div className="flex flex-col space-y-2 mt-2">
              ...
              <div className="bg-gray-800/50 p-3 rounded-lg space-y-2 text-sm">
                <div className="flex justify-between">
                  <span>Developer:</span>
                  <span className="text-blue-400">Artyom Popov</span>
                </div>
                <div className="flex justify-between">
                  <span>Contact:</span>
                  <a 
                    href="https://t.me/popov_artyom" 
                    target="_blank" 
                    rel="noopener noreferrer"
                    className="text-purple-400 hover:text-purple-300 underline"
                  >
                    https://t.me/popov_artyom
                  </a>
                </div>
              </div>
            </div>
      ...
      {/* Auth Modal */}
      {showAuthModal && (
        <AuthModal onAuth={() => setShowAuthModal(false)} />
      )}
    </div>
  );
}